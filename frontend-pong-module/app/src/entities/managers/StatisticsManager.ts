...
export class	StatisticsManager {
  ...
  updateStatistics (ball: Ball, players: Player[]) {
	...
	if (ball.maxBounceCountRound > this._gameHistory.maxBounceCount)
	...
	const	goalsConceded = players.reduce((prev, current) => {
		return (prev.history.goalsConceded > current.history.goalsConceded) ? prev : current;
	});
	this._gameHistory.mostGoalsConcededPlayer = {
		name: goalsConceded.name,
		goalsConceded: goalsConceded.history.goalsConceded,
	};
