import { BaseComponent } from "../frameworks/base-component";
import { User, UserContext } from "../globalstate/GlobalState";
import { logoutUser } from "../services/authService";
export class DropdownProfile extends BaseComponent<{ isDrop: boolean, avatar: string,name:string,user: User | null }> {
  constructor() {
    super({ isDrop: false, avatar: "https://localhost:4433/uploads/1-avatartest.jpg",name:"", user: null });
  }
  setIsDrop() {
    this.state.isDrop = !this.state.isDrop
    this.render()
  }
  setAvatar() {
    this.state.avatar = this.state.user?.avatar || "https://localhost:4433/uploads/1-avatartest.jpg";
  }
  setName() {
    this.state.name = this.state.user?.name || "no login";
  }
  /**
   * @description set the event listener (Global) for the custom event
   */
  private wsProfileListener = (e: Event) => {
    const customEvent = e as CustomEvent;
          console.log('profile-data-updated event received',customEvent.detail);
          this.state.user = customEvent.detail.profileData;
          this.setAvatar();
          this.setName();
          this.render();
  }

  connectedCallback() {
        this.state.user = UserContext().user();
        this.setAvatar();
        this.setName();
        this.render();
        this.listenCustomEvent('profile-data-updated', this.wsProfileListener);
  };


  render() {
    const { isDrop, avatar,name,user } = this.state;
    if (!user) {
      this.innerHTML = ``;
      return;
    }
       

    this.innerHTML = `
        <div  > 
    ${!isDrop ? `<div id="dropdownMenu" class="w-20 nav-link">
            <button type="button" class="flex text-sm bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
<span class="sr-only">Open user menu</span>
<img referrerPolicy="no-referrer" class="w-8 h-8 rounded-full" src=${avatar} alt="user photo">
</button>
        </div> `: `
        <div class="z-50 flex flex-col my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
        <div class="flex flex-row w-60">
              <div class="px-4 py-3">
              
             
           <img  referrerPolicy="no-referrer" id="dropdownMenu" class=" rounded-full" src=${avatar} alt="user photo">
              
              
              </div>
            <div class="px-4 py-3">
                <span class="block text-sm ">${name}</span>
                <msg-btn></msg-btn>
            </div>
        </div>
            <ul class="py-2" aria-labelledby="user-menu-button">
            <li>
            
            <a href="/dashboard" class=" flex flex-row justify-content items-center block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
            <svg fill="currentColor" class="w-4 h-4 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            // <!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
            <path d="M0 96C0 60.7 28.7 32 64 32l384 0c35.3 0 64 28.7 64 64l0 320c0 35.3-28.7 64-64 64L64 480c-35.3 0-64-28.7-64-64L0 96zm64 64l0 256 160 0 0-256L64 160zm384 0l-160 0 0 256 160 0 0-256z"/></svg>
            <span  class="px-2 ">dashboard</span>
            </a>
            </li>
            <li>
            
            <a href="/game-setting" class=" flex flex-row justify-content items-center block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
            <svg fill="currentColor" class="w-4 h-4 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            // <!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
             <path d="M495.9 166.6c3.2 8.7 .5 18.4-6.4 24.6l-43.3 39.4c1.1 8.3 1.7 16.8 1.7 25.4s-.6 17.1-1.7 25.4l43.3 39.4c6.9 6.2 9.6 15.9 6.4 24.6c-4.4 11.9-9.7 23.3-15.8 34.3l-4.7 8.1c-6.6 11-14 21.4-22.1 31.2c-5.9 7.2-15.7 9.6-24.5 6.8l-55.7-17.7c-13.4 10.3-28.2 18.9-44 25.4l-12.5 57.1c-2 9.1-9 16.3-18.2 17.8c-13.8 2.3-28 3.5-42.5 3.5s-28.7-1.2-42.5-3.5c-9.2-1.5-16.2-8.7-18.2-17.8l-12.5-57.1c-15.8-6.5-30.6-15.1-44-25.4L83.1 425.9c-8.8 2.8-18.6 .3-24.5-6.8c-8.1-9.8-15.5-20.2-22.1-31.2l-4.7-8.1c-6.1-11-11.4-22.4-15.8-34.3c-3.2-8.7-.5-18.4 6.4-24.6l43.3-39.4C64.6 273.1 64 264.6 64 256s.6-17.1 1.7-25.4L22.4 191.2c-6.9-6.2-9.6-15.9-6.4-24.6c4.4-11.9 9.7-23.3 15.8-34.3l4.7-8.1c6.6-11 14-21.4 22.1-31.2c5.9-7.2 15.7-9.6 24.5-6.8l55.7 17.7c13.4-10.3 28.2-18.9 44-25.4l12.5-57.1c2-9.1 9-16.3 18.2-17.8C227.3 1.2 241.5 0 256 0s28.7 1.2 42.5 3.5c9.2 1.5 16.2 8.7 18.2 17.8l12.5 57.1c15.8 6.5 30.6 15.1 44 25.4l55.7-17.7c8.8-2.8 18.6-.3 24.5 6.8c8.1 9.8 15.5 20.2 22.1 31.2l4.7 8.1c6.1 11 11.4 22.4 15.8 34.3zM256 336a80 80 0 1 0 0-160 80 80 0 1 0 0 160z"/></svg>
            <span  class="px-2 ">Settings</span>
            </a>
            </li>
            <li>
                <a href="/profile" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">profile</a>
            </li>
            <li id="logoutBtn">
                <a href="/" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Sign out</a>
            </li>
            </ul>
       </div>
`}`




//TODO make test
//BUG make test
    this.attachEvent(this, '#dropdownMenu', 'click', this.setIsDrop.bind(this));
    const logoutBtn = this.querySelector('#logoutBtn') as HTMLElement;
    if (logoutBtn) {
      this.attachEvent(this, '#logoutBtn', 'click',  this.handleSubmitLogout.bind(this));
    }
  }
  /**
   * logoutUser
   */
    handleSubmitLogout = async (e: Event) => {
      e.preventDefault();
      console.log('handleSubmitLogout');
      try {
        await  logoutUser(); 
        UserContext().setUserLogout();      
      } catch (error) {
        console.error('logoutUser failed:', error);
      }
   };
};