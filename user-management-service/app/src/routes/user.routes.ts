...
/* ,
  ...
  //path.join("/app/src", 'uploads', data.filename);
 // const image = Buffer.from(data.data, 'base64');
  await pump(data.file, createWriteStream(uploadPath));
  await userController.updateUser({ id: '3' }, { avatar: uploadPath });

  return reply.send({ message: 'Fichier uploadé avec succès', filename:uploadPath });
} );*/
