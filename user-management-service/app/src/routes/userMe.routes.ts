...
// manipuler les donnees liees a l'utilisateur connecte
  ...
  */
  app.get('/',{schema:  UserMeSchema.getUserMe , preHandler: [verifyAuth] }, userController.getUserMe) /* async function (req, reply) {

  /*
   metre a jour les donnees de l'utilisateur connecté
   */
  app.put<{ Body: UpdateUserBody }>("/", {schema:  UserMeSchema.updateUserMe , preHandler: [verifyAuth,verifyCSRF] },userController.updateMe);
  app.delete("/", {schema: UserMeSchema.deleteUserMe, preHandler: [verifyAuth,verifyCSRF] }, userController.deleteUserMe);
...
//  app.get("/invitations/:id",/*  {schema: UserSchema.getUserById} ,*/ userController.getUserInvitationById);
  app.get<{ Params: { id: string; }; }>("/users/:id",{schema:UserMeSchema.getUserMeById, preHandler: [verifyAuth] },/*  {schema: UserSchema.getUserById} ,*/ userController.getUserMeById); 
  
/* 
   recuperer les stats de l'utilisateur connecté
  */

  app.get("/leaderboard", {schema: UserMeSchema.getUsersLeaderboard} , userController.getUsersLeaderboard); 
  app.get("/stats/:id", {schema: UserMeSchema.getUserStatsById} , userController.getUserStatsById); 
  /* metre a jour les amis de l'utilisateur connecté*/
 // app.put("/addFriend", {schema: UserMeSchema.addFriend}, userController.addFriend);//@TODO : non use
  app.put<{ Body: { friendName: string; }; }>("/addFriendByUserName",{schema: UserMeSchema.addFriendByUserName, preHandler: [verifyAuth] }, userController.addFriendByUserName);
  app.put<{ Body: { friendId: string; }; }>("/removeFriendById", {schema: UserMeSchema.removeFriendById,preHandler: [verifyAuth] }, userController.removeFriend);
  app.put("/updatePassword",{ schema: UserMeSchema.updateMePassword,preHandler: [verifyAuth] }, userController.updateMePassword);
 ...
 // app.post("/query", {schema: UserSchema.requestQuery}, userController.requestQuery);
   ...
   */
  app.get("/games",{ schema: GamesSchema.getGames, preHandler: [verifyAuth] }, userController.getUserGames);
  app.get<{ Params: { id: string; }; }>("/games/:id",{schema: GamesSchema.getUserGamesByPlayerId, preHandler: [verifyAuth] }, userController.getUserGamesByPlayerId);
 // app.get("/games/:id",/*  {schema: UserSchema.getUserById} ,*/ userController.getUserGameById);
   ...
   */
  app.get("/tournaments",{ schema: GamesSchema.getTournaments, preHandler: [verifyAuth] },/*  {schema: UserSchema.getUserById} ,*/ userController.getUserTournaments);
  app.get<{ Params: { id: string; }; }>("/tournaments/:id",{ schema: GamesSchema.getUserTournamentsByUserId, preHandler: [verifyAuth] },/*  {schema: UserSchema.getUserById} ,*/ userController.getUserTournamentsByUserId);
...
//  app.get("/tournaments/id/:id/rounds/:roundId/stats",/*  {schema: UserSchema.getUserById} ,*/ userController.getUserTournamentRoundStatsById);
  ...
  app.get('/2fa/status', {schema: UserMeSchema.get2FAStatus, preHandler: [verifyAuth] }, get2FAStatus);
  // Activer le 2FA
  app.put('/2fa/enable', {schema:UserMeSchema.enable2FA, preHandler: [verifyAuth] }, enable2FA);
  // Désactiver le 2FA
  app.put('/2fa/disable', {schema:UserMeSchema.disable2FA, preHandler: [verifyAuth] }, disable2FA);
  // Générer un QR code pour le 2FA
  app.get('/2fa/qrcode', {schema: UserMeSchema.get2FAQRCode, preHandler: [verifyAuth] }, generate2FAQrCode);
